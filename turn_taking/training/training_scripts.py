...
def K_folds(top_prefix, batch_size, learning_rate, loss_fn, cfg, n_folds=5):
    ...
    for fold in [f'fold_{i}' for i in range(0,n_folds)]:
        ...
        # dataset:  train
        audio_dataset = AudioDataset(pickle_file, mode=cfg['model_cfg']['mode'], wavdir=wavdir)
        dataloader = DataLoader(audio_dataset, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=8, prefetch_factor=4)

        # dataset:  val
        val_audio_dataset = AudioDataset(val_pickle_file, mode=cfg['model_cfg']['mode'], wavdir=wavdir)
...
if __name__ == "__main__":    
    ...
    corpus = 'switchboard'
    logdir = "/data/ssd1/russelsa/debugging"
    ...
    top_prefix = f"runs_{cfg['model_cfg']['mode']}_{corpus}_crosstalk_removed/{timestamp}"
    top_prefix = os.path.join(logdir, top_prefix)

    # train K folds
    folds_dir = f"dataset_management/dataset_manager/assets/new_folds/{corpus}_phonwords"
