...
Reads:
  raw_data/mediapipe_output/landmarks_left.pkl   (speaker 0)
  raw_data/mediapipe_output/landmarks_right.pkl  (speaker 1)

Writes:
  raw_data/mediapipe_output/synced_video_60feat_left.pkl    (T, 60)  per speaker
  raw_data/mediapipe_output/synced_video_60feat_right.pkl   (T, 60)
  raw_data/mediapipe_output/synced_video_60feat_stereo.pkl  (T, 60, 2)
        ^ this is the format the model expects: {'arr', 'columns', 'fps'}
...
def derive_aus_from_blendshapes(df_in: pd.DataFrame) -> np.ndarray:
    ...
    if not all(c in df_in.columns for c in bs_cols):
        # blendshapes missing — fall back to zeros so the script still runs
        return np.zeros((len(df_in), len(AU_COLS)), dtype=np.float32)
...
def build_one_speaker(df_in: pd.DataFrame, label: str) -> pd.DataFrame:
    ...
    print(f"[{label}] AU non-zero frame fraction: {nonzero_frac:.3f}", flush=True)
...
def main():
    # Reads the blendshape-augmented landmarks pkls written by
    # mediapipe_feature_extraction.py. Outputs land in *_bs.pkl files so the
    # earlier AU=0 versions (synced_video_60feat_*.pkl, synced_video_60feat_stereo.pkl)
    # stay around for comparison.
    in_left  = Path("buse_workspace/session02/mediapipe_output/landmarks_left_bs.pkl")
    in_right = Path("buse_workspace/session02/mediapipe_output/landmarks_right_bs.pkl")
    out_dir  = Path("buse_workspace/session02/mediapipe_output")
    out_dir.mkdir(parents=True, exist_ok=True)

    out_left   = out_dir / "synced_video_60feat_left_bs.pkl"
    out_right  = out_dir / "synced_video_60feat_right_bs.pkl"
    out_stereo = out_dir / "synced_video_60feat_stereo_bs.pkl"
    ...
    df_l = build_one_speaker(dl["df"], "left")
    df_r = build_one_speaker(dr["df"], "right")
